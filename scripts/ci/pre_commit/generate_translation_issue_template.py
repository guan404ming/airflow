...
def update_translation_files_options(issue_template_path: Path, json_files: list[Path]) -> None:
    with open(issue_template_path, encoding="utf-8") as f:
        data = yaml.safe_load(f)
    for item in data.get("body", []):
        if item.get("id") == "translation_files" and item.get("type") == "checkboxes":
            item["attributes"]["options"] = [{"label": f.name} for f in json_files]
...
if __name__ == "__main__":
    EN_LOCALE_DIR = AIRFLOW_CORE_SOURCES_PATH / "airflow/ui/src/i18n/locales/en"
    ISSUE_TEMPLATE_PATH = AIRFLOW_ROOT_PATH / ".github/ISSUE_TEMPLATE/7-translation.yml"
    json_files = sorted(EN_LOCALE_DIR.glob("*.json"))
    update_translation_files_options(ISSUE_TEMPLATE_PATH, json_files)
